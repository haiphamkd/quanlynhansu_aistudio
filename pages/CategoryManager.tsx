...
import { Settings, Plus, Pencil, Trash2, X, Save, Tag, Layers, GraduationCap, FileQuestion, MapPin, Activity, Building } from 'lucide-react';
...
const CategoryManager: React.FC = () => {
  ...
  const categoryTypes = [
    ...
    { code: 'KhoaPhong', label: 'Khoa Phòng', icon: Building }, // Added Khoa Phong
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingCat) {
      await dataService.updateCategory({ ...editingCat, ...formData });
    } else {
         ...
         <GenericTable<Category>
            ...
            actions={(item) => (
               <div className="flex space-x-2 justify-end">
                  <button onClick={() => handleEdit(item)} className="p-1.5 text-gray-500 hover:text-blue-600 bg-gray-50 hover:bg-blue-50 rounded"><Pencil size={16}/></button>
                  <button onClick={() => handleDelete(item.id)} className="p-1.5 text-gray-500 hover:text-red-600 bg-gray-50 hover:bg-red-50 rounded"><Trash2 size={16}/></button>
               </div>
            )}
         />
      </div>
           ...
           <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                 <div>
                    ...
                    <select 
                       value={formData.type} 
                       disabled={true} // Lock to current tab
                 ...
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Giá trị hiển thị</label>
                    <input 
                       type="text" required 
                       value={formData.value}
                       onChange={e => setFormData({...formData, value: e.target.value})}
                       className="block w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-teal-500"
                 ...
                 <div className="pt-4 flex justify-end space-x-3">
                    ...
                    <AppButton type="submit" variant="primary" icon={Save}>{editingCat ? 'Cập nhật' : 'Lưu'}</AppButton>
                 </div>
              </form>
           </div>
        </div>
